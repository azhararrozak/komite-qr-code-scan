...
const RekapDataWaliKelas = () => {
  ...
  const loadClassData = async () => {
    try {
      const data = await getStudentsByClass(classAssigned, { search, status: statusFilter })
      calculateStats(data)
    } catch (err) {
  ...
  const getProgressPercentage = () => {
    return classStats.totalTarget > 0 ? Math.round((classStats.totalPaid / classStats.totalTarget) * 100) : 0
  }
    ...
    return (
      <div className="p-6">
        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded">
          Anda belum ditugaskan untuk mengelola kelas tertentu. Hubungi administrator.
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Rekap Data Kelas {classAssigned}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-green-50 p-6 rounded-lg">
          ...
          <p className="text-2xl font-bold text-green-900">{formatRupiah(classStats.totalPaid)}</p>
        </div>
        <div className="bg-orange-50 p-6 rounded-lg">
          <h3 className="text-sm font-medium text-orange-600">Target</h3>
          <p className="text-2xl font-bold text-orange-900">{formatRupiah(classStats.totalTarget)}</p>
        </div>
        <div className="bg-purple-50 p-6 rounded-lg">
          <h3 className="text-sm font-medium text-purple-600">Progress</h3>
          <p className="text-2xl font-bold text-purple-900">{getProgressPercentage()}%</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-green-50 p-6 rounded-lg">
          ...
          <p className="text-3xl font-bold text-green-900">{classStats.lunas}</p>
        </div>
        <div className="bg-yellow-50 p-6 rounded-lg">
          <h3 className="text-sm font-medium text-yellow-600">Belum Lunas</h3>
          <p className="text-3xl font-bold text-yellow-900">{classStats.belumLunas}</p>
        </div>
        <div className="bg-red-50 p-6 rounded-lg">
          <h3 className="text-sm font-medium text-red-600">Belum Bayar</h3>
          <p className="text-3xl font-bold text-red-900">{classStats.belumDibayar}</p>
        </div>
      </div>
